...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/16 19:15:00 by aplotnyk          #+#    #+#             */
/*   Updated: 2023/12/16 19:15:00 by aplotnyk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * val << || >> stack_B ->return biggst_el_ind from b(curr_elem to replace it)
 * val < B_last elemb || > 1st B_elem - its future index is 0;
 * when cases with future_index=0 and f_i=max, are checked so now start from 1
 * stop while loop when: next=NULL && value < curr_pointer to b and > next b; * 
*/
int	find_future_index_in_b(int value, t_stack *stack_b)
{
	...
	if (value < get_last_elem(stack_b)->data && value > stack_b->data)
		return (0);
	future_index = 1;
